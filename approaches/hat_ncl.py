    ...
    def train(self,t,xtrain,ytrain,xvalid,yvalid,args):
        ...
        for n,_ in self.model.named_parameters():
            ...
            if vals is not None:
                ...
                network_usage += self.mask_back[n].numel() - self.mask_back[n].round().nonzero().size(0)
                network_capacity += (torch.ones(self.mask_back[n].size())).sum()
                network_active += self.mask_back[n].round().nonzero().size(0) #current active

        print('network_usage: ',network_usage)
        print('network_capacity: ',network_capacity)
        print('network_active: ',network_active)

        self.network_capacity_usage[self.epoch_np] = 1.0 * network_usage / network_capacity
        self.network_active[self.epoch_np] = 1.0 * network_active / network_capacity.item()

        self.epoch_np+=1
        print('self.network_capacity_usage: ',self.network_capacity_usage)
        print('self.network_active: ',self.network_active)

        np.savetxt(args.output + '_network_capacity_usage',self.network_capacity_usage,'%.4f',delimiter='\t')
        np.savetxt(args.output + '_network_active',self.network_active,'%.4f',delimiter='\t')
