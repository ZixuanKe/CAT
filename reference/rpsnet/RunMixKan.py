...
for t,ncla in taskcla:
    ...
    inp = np.sum([p.numel() for p in net.mcl.fc1.parameters()]).item()
    hid = np.sum([p.numel() for p in net.mcl.fc2.parameters()]).item()
    kta = np.sum([p.numel() for p in net.kt.encoder.parameters()]).item()
    emb = np.sum([p.numel() for p in net.mcl.efc1.parameters()]).item() + np.sum([p.numel() for p in net.mcl.efc2.parameters()]).item() + np.sum([p.numel() for p in net.kt.q1.parameters()]).item()
    
    hea = np.sum([p.numel() for p in net.mcl.mask_last.parameters()]).item() + np.sum([p.numel() for p in net.mcl.att_last.parameters()]).item()
    tra = np.sum([p.numel() for p in net.transfer.transfer.parameters()]).item() + np.sum([p.numel() for p in net.mcl.mask_last.parameters()]).item() + inp + hid
    
    ref = np.sum([p.numel() for p in net.transfer.fc1.parameters()]).item() + np.sum([p.numel() for p in net.transfer.fc2.parameters()]).item() + np.sum([p.numel() for p in net.transfer.last.parameters()]).item() + np.sum([p.numel() for p in net.transfer.transfer.parameters()]).item()
    ref = tra / 2
    print ('input:', inp/1e6)
    print ('hidden:', hid/1e6)
    print ('kta:', kta/1e6)
    print ('task id embedding:', emb/1e6)
    print ('heads:', hea/1e6)
    print ('transfer:', tra/1e6)
    print ('reference:', ref/1e6)
    print ('total:', (inp+hid+kta+emb+hea+tra+ref)/1e6)
    ...
    for candidate_phase in candidate_phases:
        ...
        if 'pipeline' in args.note:
            ...
            if candidate_phase == 'mcl' and 'auto' in args.note and 'multi-loss-joint' in args.note:
                similarity,ranking = read_baseline_similarity()
            elif candidate_phase == 'mcl' and 'auto' in args.note:
                similarity,ranking = auto_similarity(task,appr)
            elif candidate_phase == 'mcl' and 'by-name' in args.note:
                similarity = name_similarity(task,norm_transfer_raw,data)
            elif candidate_phase == 'mcl' and 'all-one' in args.note:
                print('all-one')
                similarity,ranking = all_one_similarity(task,norm_transfer_raw,data)
        ...
        check_federated.set_ranking(rankings)
