...
def get(seed=0,fixed_order=False,pc_valid=0.10,remain=0,args=0):
    data={}
    taskcla=[]
    size=[1,28,28]
    ...
    # number of example
    # x = FEMMNISTTrain(root_dir='./dat/femnist/'+data_type+'/iid/train/',transform=transforms.Compose([transforms.ToTensor(),transforms.Normalize(mean,std)]))
    # y=torch.utils.data.DataLoader(x,batch_size=1,shuffle=True)
    # print(len([0 for user, image, target in y]))
    #
    # x = FEMMNISTTest(root_dir='./dat/femnist/'+data_type+'/iid/test/',transform=transforms.Compose([transforms.ToTensor(),transforms.Normalize(mean,std)]))
    # y=torch.utils.data.DataLoader(x,batch_size=1,shuffle=True)
    # print(len([0 for user, image, target in y]))
    #
    # number of example
