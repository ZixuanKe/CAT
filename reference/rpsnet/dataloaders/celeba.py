...
def get(seed=0,pc_valid=0.10,args=0):
    ...
    if not os.path.isdir('./dat/'+data_type+'_binary_celeba/'):
        os.makedirs('./dat/'+data_type+'_binary_celeba')
        ...
        for n,user in enumerate(users):
            for s in ['train','test']:
                ...
                torch.save(data[n][s]['x'], os.path.join(os.path.expanduser('./dat/'+data_type+'_binary_celeba/'),'data'+str(n)+s+'x.bin'))
                torch.save(data[n][s]['y'], os.path.join(os.path.expanduser('./dat/'+data_type+'_binary_celeba/'),'data'+str(n)+s+'y.bin'))
    ...
    for i in range(num_task):
        ...
        for s in ['train','test']:
            data[i][s]={'x':[],'y':[]}
            data[i][s]['x']=torch.load(os.path.join(os.path.expanduser('./dat/'+data_type+'_binary_celeba/'),'data'+str(ids[i])+s+'x.bin'))
            data[i][s]['y']=torch.load(os.path.join(os.path.expanduser('./dat/'+data_type+'_binary_celeba/'),'data'+str(ids[i])+s+'y.bin'))
