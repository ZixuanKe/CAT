...
def get(seed=0,pc_valid=0.10,args=0):
    ...
    if not os.path.isdir('./dat/binary_cifar100/'):
        ...
        for s in ['train','test']:
            ...
            for image,target in loader:
                ...
                if counter[n] > n_per_task-reduce: # we do want to reduce some amount of examples
                    continue
    ...
    # std=[x/255 for x in [63.0,62.1,66.7]]
    #
    # x=datasets.CIFAR100('./dat/',train=True,download=True,transform=transforms.Compose([transforms.ToTensor(),transforms.Normalize(mean,std)]))
    # y=torch.utils.data.DataLoader(x,batch_size=1,shuffle=False)
    # print(len([0 for image, target in y]))
    #
    # x=datasets.CIFAR100('./dat/',train=False,download=True,transform=transforms.Compose([transforms.ToTensor(),transforms.Normalize(mean,std)]))
    # y=torch.utils.data.DataLoader(x,batch_size=1,shuffle=False)
    # print(len([0 for image, target in y]))
    # number of example
