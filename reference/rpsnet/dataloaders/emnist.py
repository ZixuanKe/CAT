...
def get(seed=0,fixed_order=False,pc_valid=0,args=0):
    ...
    # number of example
    # x = datasets.EMNIST('./dat/',train=True,download=True,split='balanced',transform=transforms.Compose([transforms.ToTensor(),transforms.Normalize(mean,std)]))
    # y = torch.utils.data.DataLoader(x,batch_size=1,shuffle=True)
    # print(len([0 for image, target in y]))
    #
    # x = datasets.EMNIST('./dat/',train=False,download=True,split='balanced',transform=transforms.Compose([transforms.ToTensor(),transforms.Normalize(mean,std)]))
    # y = torch.utils.data.DataLoader(x,batch_size=1,shuffle=True)
    # print(len([0 for image, target in y]))
    #
