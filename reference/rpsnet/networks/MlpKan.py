...
class Net(torch.nn.Module):
    ...
    def __init__(self,inputsize,taskcla,nlayers=2,nhid=2000,pdrop1=0.2,pdrop2=0.5,args=None):
        ...
        self.transfer = TransferLayer(self.taskcla,nhid,ncha,size)
    ...
    def forward(self,t,x,s=1,phase=None,
        ...
        if 'mcl' in phase and 'baseline' in self.args.note:
        ...
        elif 'mcl' in phase and 'multi-loss-joint-Tsim' in self.args.note:
            ...
            if len(pre_models) > 1:
                ...
                if 'sim-ranking' in self.args.note:
                    h_attn,_ = self.kt.encoder(task_models,query,check_federated.get_ranking(pre_ts,cand_t))
                else:
...
class EncoderLayer(nn.Module):
    ...
    def forward(self, enc_input, enc_q=None,ranking=None):
        ...
        if 'sim-ranking' in self.args.note:
            enc_input = self.dropout(self.position_enc(enc_input,ranking))
