...
class Appr(object):
    ...
    def __init__(self,model,nepochs=100,sbatch=64,lr=0.05,lr_min=1e-4,lr_factor=3,lr_patience=5,clipgrad=10000,lamb=0.75,smax=400,
                 nepochs_kt=300,lr_kt=0.025,lr_patience_kt=10,
                 args=None):
        self.model=model
        ...
        self.nepochs_kt=nepochs_kt
        self.lr_kt=lr_kt
        self.lr_patience_kt=lr_patience_kt
    ...
    def _get_optimizer(self,lr=None,phase=None,args=None):
        ...
        elif phase=='mcl' and ('pipeline' in args.note or 'baseline' in args.note):
            return torch.optim.SGD(list(self.model.mcl.parameters()),lr=lr)

        elif phase=='mcl' and 'joint' in args.note:
    ...
    def eval(self,t,x,y,phase=None,
        ...
        for i in range(0,len(r),self.sbatch):
            ...
            if phase=='mcl' and 'baseline' not in self.args.note and outputs_attn is not None:
        ...
        if 'all-one' in self.args.note:
                print('all-one')
                total_loss = total_att_loss
                total_acc = total_att_acc

        elif phase=='mcl' and 'baseline' not in self.args.note:
            print('total_att_acc: ',total_att_acc/total_num)
            print('total_mask_acc: ',total_mask_acc/total_num)
...
class CheckFederated():
    ...
    def set_ranking(self,rankings):
        print('set rankings: ',rankings)
        self.rankings = rankings

    def get_similarities(self):
        return self.similarities

    def get_ranking(self,pre_ts,t):
        print('pre_ts: ',pre_ts)
        print('t: ',t)
        print('get ranking[t]: ',self.rankings[t])
        return self.rankings[t][torch.from_numpy(np.array(pre_ts)).long()]
