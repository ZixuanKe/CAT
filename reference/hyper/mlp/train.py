...
from mlp import train_utils as tutils
    ...
    with torch.no_grad():
        ...
        while N_processed < test_size:
            ...
            if config.cl_scenario == 1:
                # Select current head.
                task_out = [sum(config.dims[:task_id]), sum(config.dims[:task_id+1])]
...
def train(task_id, data, mnet, hnet, device, config, shared, writer, logger):
    ...
    if calc_reg:
        ...
        if config.cl_scenario != 2:
            ...
            regged_outputs = [list(range(sum(config.dims[:task_id]), sum(config.dims[:task_id+1]))) for i in
                              range(task_id)]
    ...
    for i in range(training_iterations):
        ...
        if config.soft_targets:
            soft_label = 0.95
            num_classes = config.dims[task_id]
...
def run(config, experiment='mlp'):
    ...
    """
    assert(experiment in ['resnet', 'zenke','mlp'])
    ...
    for j in range(config.num_tasks):
        ...
        print('mnet: ')
        print_model_report(mnet)

        print('hnet: ')
        print_model_report(hnet)
