    ...
    def logit_cross_entropy_loss(h, t, reduction='mean'):
        ...
        if type(h) != list:
            ...
            for i in range(len(h)):
                targets = t.argmax(dim=1, keepdim=False)
                loss+=F.cross_entropy(h[i], targets, reduction=reduction)
            return loss
    ...
    def knowledge_distillation_loss(logits, target_logits, target_mapping=None,
        ...
        if type(logits) != list:
            ...
            for i in range(len(logits)):
                targets = F.softmax(target_logits[i] / T, dim=1)
                n_classes = logits[i].shape[1]
                n_targets = targets.shape[1]
                ...
                if target_mapping is None:
                    ...
                    new_targets = torch.zeros_like(logits[i]).to(device)
                    new_targets[:, target_mapping] = targets
                    targets = new_targets

                loss += -(targets * F.log_softmax(logits[i] / T,dim=1)).sum(dim=1).mean()*\
                       T**2
    ...
    def accuracy(y, t):
        ...
        if type(y) != list:
            ...
            for i in range(len(y)):
                predictions.append(y[i].argmax(dim=1, keepdim=False))
                targets.append(t[i].argmax(dim=1, keepdim=False))
