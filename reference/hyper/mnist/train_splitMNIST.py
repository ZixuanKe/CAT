...
def _save_performance_summary(config, train_iter=None):
    ...
    with open(os.path.join(config.out_dir,
                           'forward'), 'w') as f:
        accs = [acc.item()*0.01 for acc in config.during_accs_final]
        print('accs: ',accs)
        for acc in accs:
            f.write(str(acc) + '\n')
    ...
    with open(os.path.join(config.out_dir,
                           'backward'), 'w') as f:
        accs = [acc.item()*0.01 for acc in config.overall_acc_list]
        print('accs: ',accs)
        for acc in accs:
            f.write(str(acc) + '\n')
                ...
                # compute targets - this is a bit unelegant, cl 3 requires hacks
                    ...
                    if config.cl_scenario == 1:
                        ...
                        if 'reference' in config.note:
                            ...
                            task_out = [sum(config.out_dim[:t]), sum(config.out_dim[:t+1])]
                        ...
                        for e in entrop_to_test:
                            ...
                            if config.cl_scenario == 2:
                                ...
                                if 'reference' in config.note:
                                    ...
                                    task_out = [sum(config.out_dim[:e]), sum(config.out_dim[:e+1])]
...
def get_fake_data_loss(dhandlers_rp, net, dec, d_hnet, device, config, writer, 
        ...
        if config.class_incremental or config.training_task_infer:
            ...
            if config.cl_scenario == 1:
                ...
                if 'reference' in config.note:
                    ...
                    task_out = [sum(config.out_dim[:re]), sum(config.out_dim[:re+1])]
            ...
            print('Y_hat_ls.shape: ',Y_hat_ls.shape)
    ...
    # cat to one tensor
    print('all taegets: ',[x.size() for x in all_targets])
    print('all all_Y_hat_ls: ',[x.size() for x in all_Y_hat_ls])

    if 'mixemnist' in config.note and 'sep-emnist' in config.note:
        for targets_id,targets in enumerate(all_targets):
            if targets.size(-1) != config.classptask:
                new_targets = torch.FloatTensor(targets.size(0), config.classptask).cuda()
                new_targets.zero_()
                new_targets[:,:targets.size(-1)] = targets.clone()
                all_targets[targets_id] = new_targets

        for targets_id,targets in enumerate(all_Y_hat_ls):
            if targets.size(-1) != config.classptask:
                new_targets = torch.FloatTensor(targets.size(0), config.classptask).cuda()
                new_targets.zero_()
                new_targets[:,:targets.size(-1)] = targets.clone()
                all_Y_hat_ls[targets_id] = new_targets

    elif 'mixemnist' in config.note:
        femnsit_class=62
        for targets_id,targets in enumerate(all_targets):
            if targets.size(-1) != 62:
                new_targets = torch.FloatTensor(targets.size(0),62).cuda()
                new_targets.zero_()
                new_targets[:,:targets.size(-1)] = targets.clone()
                all_targets[targets_id] = new_targets
...
def run(mode='split'):  
    ...
    print('train num_tasks: ',config.num_tasks)
    print('train cl_scenario: ',config.cl_scenario)
