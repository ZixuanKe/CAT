...
def load_datasets(config, shared, logger, data_dir='../datasets'):
    ...
    if 'reference' in config.note:
        ...
        dhandlers = get_split_cifar_handlers(data_dir, use_one_hot=True,validation_size=500,
        ...
        from data.special.my_split_cifar import get_split_cifar_handlers
        ...
        assert(config.num_tasks <= 11)
        logger.info('Loading CIFAR datasets ...')
        dhandlers = get_split_cifar_handlers(data_dir, use_one_hot=True,data=data,config=config)
        assert(len(dhandlers) == config.num_tasks)
