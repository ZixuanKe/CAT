...
def get_split_handlers(data_path, use_one_hot=False,data=0,config=0,dims=0):
    ...
    for i in range(config.num_tasks):
        handlers.append(SplitCIFAR100Data(data_path,
            use_one_hot=use_one_hot, data=data[i], dims=dims[i],config=config))
...
class SplitCIFAR100Data(CIFAR100Data):
    ...
    """
    def __init__(self, data_path, use_one_hot=False,full_out_dim=False,data=0,config=0,dims=0):
        ...
        train_labels = data['train']['y'].view(-1,1)
        valid_labels = data['valid']['y'].view(-1,1)
        test_labels = data['test']['y'].view(-1,1)
