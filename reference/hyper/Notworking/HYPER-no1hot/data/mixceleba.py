...
def get(seed=0, fixed_order=False, pc_valid=0, args=0):
    ...
    if 'stack' in args.note:
        for stack_id in range(args.ntasks):
            data[stack_id] = data_cifar100[stack_id]
            taskcla.append(taskcla_cifar100[stack_id])

        for stack_id in range(args.ntasks,args.ntasks*2):
            data[stack_id] = data_celeba[stack_id-n_tasks]
            taskcla.append((stack_id,data_celeba[stack_id-n_tasks]['ncla']))
    ...
    elif 'sep-cifar100' in args.note and 'random' in args.note:
        print('sep-cifar100-random')

        all_cifar100 = [data_cifar100[x]['name'] for x in range(args.ntasks)]

        f_name = '/home/zixuan/KAN/image/mixceleba_random_'+str(args.ntasks*2)
        ...
        for task_id in range(args.ntasks*2):
            if 'cifar100' in random_sep[task_id]:# Even
                ...
                data[data_id] = data_cifar100[cifar100_id]
                taskcla.append((data_id,data_cifar100[cifar100_id]['ncla']))
                data_id+=1

    elif 'sep-celeba' in args.note and 'random' in args.note:
        print('sep-celeba-random')
        ...
        data_id = 0
        for task_id in range(args.ntasks*2):
            if 'celeba'in random_sep[task_id]:
                celeba_id = all_celeba.index(random_sep[task_id])
                data[data_id] = data_celeba[celeba_id]
                taskcla.append((data_id,data_celeba[celeba_id]['ncla']))
                data_id+=1
    ...
    elif 'sep-celeba' in args.note:
        print('sep-celeba')
        for stack_id in range(args.ntasks):
            data[stack_id] = data_celeba[stack_id]
            taskcla.append(taskcla_celeba[stack_id])

    elif 'sep-cifar100' in args.note:
        print('sep-cifar100')
        for stack_id in range(args.ntasks):
            data[stack_id] = data_cifar100[stack_id]
            taskcla.append(taskcla_cifar100[stack_id])
...
def read_cifar100(seed=0,pc_valid=0.10,args=0):
    ...
    print("data[t]['train']['x']: ",data[0]['train']['x'].size())
