...
def get(seed=0, fixed_order=False, pc_valid=0, args=0):
    ...
    if 'stack' in args.note:
    ...
    elif 'sep-emnist' in args.note and 'random' in args.note:
        print('sep-emnist-random')

        all_emnist = [data_emnist[x]['name'] for x in range(args.ntasks)]

        f_name = '/home/zixuan/KAN/image/mixemnist_random_'+str(args.ntasks*2)
        ...
        for task_id in range(args.ntasks*2):
            if 'emnist' in random_sep[task_id]:# Even
                emnist_id = all_emnist.index(random_sep[task_id])
                data[data_id] = data_emnist[emnist_id]
                taskcla.append((data_id,data_emnist[emnist_id]['ncla']))
                data_id+=1

    elif 'sep-femnist' in args.note and 'random' in args.note:
        print('sep-femnist-random')
        ...
        print('all_femnist: ',all_femnist)

        data_id = 0
        for task_id in range(args.ntasks*2):
            if 'fe-mnist'in random_sep[task_id]:
                femnist_id = all_femnist.index(random_sep[task_id])
                data[data_id] = data_femnist[femnist_id]
                taskcla.append((data_id,data_femnist[femnist_id]['ncla']))
                data_id+=1
    elif 'random' in args.note:
    ...
    elif 'sep-emnist' in args.note:
        print('sep-emnist')
        for stack_id in range(args.ntasks):
            data[stack_id] = data_emnist[stack_id]
            taskcla.append(taskcla_emnist[stack_id])

    elif 'sep-femnist' in args.note:
        print('sep-femnist')
        for stack_id in range(args.ntasks):
            data[stack_id] = data_femnist[stack_id]
            taskcla.append(taskcla_femnist[stack_id])
...
def read_emnist(seed=0,fixed_order=False,pc_valid=0,args=0):
    ...
    std=(0.3081,)
    dat={}
    dat['train']=datasets.EMNIST('/home/zixuan/KAN/image/dat/',train=True,download=True,split='balanced',transform=transforms.Compose([transforms.ToTensor(),transforms.Normalize(mean,std)]))
    dat['test']=datasets.EMNIST('/home/zixuan/KAN/image/dat/',train=False,download=True,split='balanced',transform=transforms.Compose([transforms.ToTensor(),transforms.Normalize(mean,std)]))
...
def read_femnist(seed=0,fixed_order=False,pc_valid=0.10,remain=0,args=0):
    ...
    # print('testing_c: ',testing_c)
    print('training len: ',len(data[label//class_per_task]['train']['x']))
    print('testing len: ',len(data[label//class_per_task]['test']['x']))
