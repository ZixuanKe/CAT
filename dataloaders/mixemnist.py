...
def read_emnist(seed=0,fixed_order=False,pc_valid=0.1,args=0):
...
def read_femnist(seed=0,fixed_order=False,pc_valid=0.10,remain=0,args=0):
    ...
    if args.dis_ntasks==10:
        train_dataset = FEMMNISTTrain(root_dir='./dat/femnist/'+data_type+'/iid/train10/',transform=transforms.Compose([transforms.ToTensor(),transforms.Normalize(mean,std)]))
        dat['train'] = train_dataset

        test_dataset = FEMMNISTTest(root_dir='./dat/femnist/'+data_type+'/iid/test10/',transform=transforms.Compose([transforms.ToTensor(),transforms.Normalize(mean,std)]))
